...
export default function CheckOut() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      const productName = String(product.itemName || product.productName || '').toLowerCase();
      ...
      const barcodes = Array.isArray(product.barcodes)
        ? product.barcodes.map((item) => String(item.barcode || '').toLowerCase()).join(' ')
        : '';
  ...
  const createCartProduct = (product) => {
    ...
    return {
      ...
      name: product.itemName || product.productName || 'Unnamed Product',
      ...
      variant: product.details?.flavor || '',
      ...
      price: Number(product.details?.sellingPrice || 0),

      costPrice: Number(product.details?.costPrice || 0),

      wholesalePrice: Number(product.details?.wholesalePrice || 0),

      minimumSellingPrice: Number(product.details?.minimumSellingPrice || 0),
      ...
      taxGroup: product.taxGroup?.name || '',
  ...
  const handleProductSelect = (product) => {
    ...
    if (cartProduct.price <= 0) {
      setError(`${cartProduct.name} does not have a valid selling price.`);
  ...
  const scanProduct = async (code) => {
    ...
    try {
      const response = await searchProducts(cleanCode);
      ...
      setError(lookupError?.message || 'Failed to fetch product.');
    } finally {
    ...
    try {
      ...
      if (scannerRef.current) {
        ...
        try {
          ...
          console.log('Previous scanner clear:', error);
        }
      ...
      try {
        ...
        console.log('Failed scanner stop:', error);
      }
      ...
      try {
        ...
        console.log('Failed scanner clear:', error);
      }
  ...
  const completeSale = async () => {
    ...
    const sale = {
      ...
      cashGiven: paymentMethod === 'cash' ? cashAmount : null,
          ...
          >
            ...
            <Stack spacing={2}>
              ...
              <Button
                ...
                disabled={cartItems.length === 0 || (paymentMethod === 'cash' && (cashGiven === '' || cashAmount < total))}
                sx={{
                  minHeight: 52
                }}
              >
